...
from rag.service.orchestration.agentic.json_utils import parse_json_object
from rag.service.orchestration.agentic.prompts import (
...
from rag.service.orchestration.agentic.state import (
    AgenticRAGState,
    append_step,
    hits_from_state,
    hits_to_state,
)
from rag.service.orchestration.direct import (
    ContextBuilder,
    InputGuardrails,
    PromptBuilder,
)
...
class AgenticRAGNodes:
        ...
        if not guardrail.allowed:
            return {
                **base_update,
                ...
                or "Sorry, I can't process that request.",
                "reasoning_steps": [
                    {
    ...
    async def followup_router(self, state: AgenticRAGState) -> dict[str, Any]:
        ...
        if enabled and active_hits and any(marker in question for marker in followup_markers):
    ...
    async def retrieval_planner(self, state: AgenticRAGState) -> dict[str, Any]:
        ...
        include_highlights = bool(
            state.get("include_highlights")
            or retrieval_mode in {"bm25", "hybrid"}
        )
        fuzziness = state.get("fuzziness")
        if fuzziness is None and any(word in lower_query for word in ["title", "called"]):
            fuzziness = "AUTO"
    ...
    async def build_context(self, state: AgenticRAGState) -> dict[str, Any]:
        ...
        top_k = int(state.get("retrieval_plan", {}).get("top_k", self.settings.default_top_k))
        context = self.context_builder.build(hits[:top_k])
        ...
        return {
            ...
            "active_chunk_ids": [
                citation["chunk_id"] for citation in citations if citation.get("chunk_id")
            ],
            ...
            try:
                data = await self._structured_json(
                    EVIDENCE_GRADER_PROMPT.format(
                        question=state["safe_query"],
                        context=self._truncate(prompt_context, 6_000),
                    ),
                    self.decision_settings,
                )
    ...
    async def answer_generator(self, state: AgenticRAGState) -> dict[str, Any]:
        ...
        if not str(context_data.get("prompt_context", "")).strip():
                ...
                data = await self._structured_json(
                    ANSWER_CRITIC_PROMPT.format(
                        ...
                        context=self._truncate(
                            str(state.get("context", {}).get("prompt_context", "")),
                            6_000,
                        ),
                        answer=state.get("answer", ""),
    ...
    async def answer_repair(self, state: AgenticRAGState) -> dict[str, Any]:
        ...
        prompt = ANSWER_REPAIR_PROMPT.format(
            question=state["safe_query"],
            context=state.get("context", {}).get("prompt_context", ""),
            answer=state.get("answer", ""),
    ...
    async def _structured_json(
        ...
        return parse_json_object(generation.text)
        ...
        return BuiltContext(
            ...
            citations=[
                Citation(
                    ...
                    start_word=int(citation["start_word"]),
                    end_word=int(citation["end_word"]),
                    start_char=int(citation["start_char"]),
                    end_char=int(citation["end_char"]),
                    score=citation.get("score"),
                    highlights=[
                        str(value) for value in citation.get("highlights", [])
                    ],
                )
            ...
            sources=[
                Source(
                    ...
                    published_date=str(source["published_date"]),
